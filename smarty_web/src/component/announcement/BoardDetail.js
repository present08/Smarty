...
function BoardDetail() {
  ...
  const handleEdit = async (board_id, updatedData) => {
    try {
      const response = await axios.put(`http://localhost:8080/notice/board/user/modify/${board_id}`, updatedData);
      if (response.data) {
        alert("게시글이 수정되었습니다.");
        const updatedBoardResponse = await axios.get(`http://localhost:8080/notice/board/user/detail/${board_id}`);
    ...
    <div className="board-detail-container">
      <div className="board-detail-content">
        ...
        <div className="board-info">
          <span>게시판 종류: {board.content_type}</span>
          <span>게시판 번호: {board.board_id}</span>
        ...
        <div className="button-group">
          {/* <button className="edit-button" onClick={() => navigate(NoticeModify)}>수정</button> */}
