...
export default function FacilityModify() {
  ...
  const handleSubmit = () => {
    putOneFacility(facility_id, modifiedFacility).then(res => {
      console.log(res)
...
const handleReset = () => {
  ...
  return (
    <div className="facilityModify">
        {modifiedFacility && 
          <div className="facilityModifyContainer">        
            <div className="facilityModifyTitle">{modifiedFacility.facility_id}</div>
            <button className="ModifyButton" onClick={handleSubmit}>등록</button>
            <button className="ResetButton" onClick={handleReset}>초기값 복원</button>

            <div className="facilityModifyContent">
