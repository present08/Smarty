...
export default function NewFacility() {
    ...
    const handleFacilityAdd = (e) => {
        ...
        setFormData(facilityForm)
        
    }

    useEffect(() => {
        postAddFacility(formData).then(data => {
            // 시설 등록에 성공하고 나면 facility_id를 반환받음
            // 이후 코트, 물품등록 여부에 따라 아래 코드 실행
            console.log("시설id : ", data)
            if (facility.court === true) {
                court.map(court => {
                ...
                court.facility_id = id
                court.facility_name = facility.facility_name
                court.court_status = facility.facility_status
                setCourt({ ...court })
                console.log("코트확인 ", court)
                console.log("else블럭진입")
                postAddCourt(court)
            }
            if (product.length > 0) {
